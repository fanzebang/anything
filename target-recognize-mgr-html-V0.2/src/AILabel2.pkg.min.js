(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.AILabel2 = factory());
  ...
  var EventLayer = /*#__PURE__*/function (_Layer) {
    ...
    _createClass(EventLayer, [{
          ...
          case EFeatureType.Circle:
            {
              ...
              forEach_1(points, function (point, index) {
                ...
                if (distance <= 5) {
                  ...
                  // if(!point.width && !point.height) _this11.map.setCursor(cursor);
                  // if(!point.width && !point.height) _this11.map.setCursor("nwse-resize");
            
                  if(point.position == 'rightCenter' ) _this11.map.setCursor("e-resize");
                  if(point.position == 'leftCenter' ) _this11.map.setCursor("w-resize");
                  if(point.position == 'topCenter' ) _this11.map.setCursor("n-resize");  
                  if(point.position == 'bottomCenter' ) _this11.map.setCursor("s-resize");   //范修改
                  if(point.position == 'leftTop' ) _this11.map.setCursor("nw-resize"); 
                  if(point.position == 'rightTop' ) _this11.map.setCursor("ne-resize"); 
                  if(point.position == 'rightBottom' ) _this11.map.setCursor("se-resize"); 
                  if(point.position == 'leftBottom' ) _this11.map.setCursor("sw-resize"); 
      ...
      value: function handleActiveFeatureMove(e) {
          ...
          case EFeatureType.Rect:
            {
              ...
              if (this.hoverFeature) {
                newRectShape = {
                  ...
                  if(this.map.platformDom.style.cursor == "e-resize"){
                    preNewWidth = width + preGlobalDltX
                    preNewY =  _y
                    preNewX = _x
                    preNewHeight = height

                  }else if(this.map.platformDom.style.cursor =="w-resize"){

                    preNewWidth = width-preGlobalDltX
                    preNewY =  _y
                    preNewX = _x + globalDltX
                    preNewHeight = height

                  }else if(this.map.platformDom.style.cursor =="n-resize"){

                    preNewWidth = width
                    preNewY =  _y - globalDltY 
                    preNewX = _x 
                    preNewHeight = height- preGlobalDltY 

                  }else if(this.map.platformDom.style.cursor =="s-resize"){
                  ...
                  }else if(this.map.platformDom.style.cursor =="nw-resize"){

                    preNewX = _isLeft ? _x + globalDltX : _x
                    preNewY = isTop ? _y - globalDltY : _y;
                    preNewWidth = _isLeft ? width - preGlobalDltX : width + preGlobalDltX;
                    preNewHeight = isTop ? height - preGlobalDltY : height + preGlobalDltY;

                  }else if(this.map.platformDom.style.cursor =="ne-resize"){
                  ...
                  }else if(this.map.platformDom.style.cursor =="se-resize"){
                    preNewX = _isLeft ? _x + globalDltX : _x
                    preNewY = isTop ? _y - globalDltY : _y;
                    preNewWidth = _isLeft ? width - preGlobalDltX : width + preGlobalDltX;
                    preNewHeight = isTop ? height - preGlobalDltY : height + preGlobalDltY;
          
                  }else if(this.map.platformDom.style.cursor =="sw-resize"){
                    preNewWidth = width-preGlobalDltX
                    preNewY =  _y
                    preNewX = _x + globalDltX
                    preNewHeight = height+ preGlobalDltY
                  ...
                  // if(!this.map.platformDom.style.cursor == "e-resize" && !this.map.platformDom.style.cursor == "w-resize"){
                  //   newHeight = Math.abs(preNewHeight);
                  // }else{
                  //   newHeight // 
                  // }
                  newRectShape = {
        ...
        if(this.map.platformDom.style.cursor == "e-resize"){
          newRectShape.height += 0
       
            // global.height += 0 
            // newRectShape.width += 1 
        }
    ...
    _createClass(RectFeature, [{
      ...
      value: function getPoints() {
        ...
        return [{
          ...
          x: startX+width/2,    // 范修改 增加矩形左-右 中间点
          y: startY,
          position:'topCenter'
        }, {
          x: endX,
          ...
          x: endX,
          y: startY+height/2,   // 范修改 增加矩形右-下 中间点
          ...
          x: endX-width/2,   // 范修改 增加矩形下-左 中间点
          y: endY,
          ...
          x: startX,
          y: endY-height/2,  // 范修改 增加矩形左-下 中间点
          position:'leftCenter'   
        }];
      } // 执行绘制当前
      // @override
